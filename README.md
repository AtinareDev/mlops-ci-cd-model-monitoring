...
[![MLOps CI/CD Pipeline](https://github.com/AtinareDev/mlops-ci-cd-model-monitoring/actions/workflows/mlops.yml/badge.svg)](https://github.com/AtinareDev/mlops-ci-cd-model-monitoring/actions/workflows/mlops.yml)
...
El proyecto automatiza el ciclo de validación del modelo, bloquea ejecuciones inválidas y publica únicamente los resultados aprobados en GitHub Pages.
...
| Workflow CI/CD | https://github.com/AtinareDev/mlops-ci-cd-model-monitoring/actions/workflows/mlops.yml |

## Descripción general

Este repositorio implementa un pipeline CI/CD para validar modelos de machine learning antes de considerarlos aptos para producción.

El flujo automatizado ejecuta controles de calidad de código, pruebas unitarias, entrenamiento del modelo, validación de métricas y generación de un reporte HTML desplegado automáticamente en GitHub Pages.

El modelo solo se considera válido cuando supera los umbrales mínimos definidos para `accuracy`, `precision` y `recall`.
...
- Proyecto documentado para portfolio técnico.
...
```bash
uv run python scripts/run_model_pipeline.py
uv run python scripts/validate_metrics.py --threshold 0.80
```
